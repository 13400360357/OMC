...
def omc_add(b):
    '1 login'
    b.find_element_by_id('uid').send_keys('admin')
    time.sleep(0.1)
    b.find_element_by_xpath("//input[contains(@id,'passw')]").send_keys('abc123')
    b.find_element_by_xpath("//span[contains(text(),'登录')]").click()
    time.sleep(1)

    try:
#         b.find_element_by_xpath("//a[contains(text(),'配置')]").click()
        llll=b.find_element_by_link_text('配置')
        llll.click()
    except:
#         b.find_element_by_xpath("//a[contains(text(),'基站')]").click()
        b.find_element_by_link_text('基站').click()
        time.sleep(0.5)
#         b.find_element_by_xpath("//a[contains(text(),'配置')]").click()        
        b.find_element_by_link_text('配置').click()
    time.sleep(1)


    '2 NodeB select'
    WebDriverWait(b, 15).until(lambda x:b.find_element_by_xpath("//input[contains(@placeholder,'基站编码/名称')]")).send_keys('12345678900987654321')
    b.find_element_by_link_text('查询').click()
    b.find_element_by_xpath("//td[@field='ck']/div/input[@type='checkbox']").click()
    ...
    for e in range(1,len(dict)+1):
        ...
        '1.进行下拉框点击' 
        aa=b.find_element_by_xpath('/html/body/div[1]/div[4]/div/div/div[2]/div/div/div[2]/div/div/div[2]/div/div[1]/div/span/span/a')
